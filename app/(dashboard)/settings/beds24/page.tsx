...
export default function Beds24Page() {
  ...
  const loadProperties = useCallback(async () => {
    ...
    if (res.ok) {
      setBeds24Properties(data.beds24Properties);
  ...
  async function connect() {
    setConnecting(true);
    setError("");
    const res = await fetch("/api/beds24/account", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function disconnect() {
    if (!confirm("Disconnect Beds24? Bookings will no longer sync automatically.")) return;
    await fetch("/api/beds24/account", { method: "DELETE" });
    setConnected(false);
    setBeds24Properties([]);
  }
  ...
  async function saveMappings() {
    ...
    const payload = stayhqProperties.map((p) => ({
      propertyId: p.id,
      beds24PropertyId: mappings[p.id] || "",
    }));
    const res = await fetch("/api/beds24/properties", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  function copyWebhookUrl() {
    const url = `${window.location.origin}/api/beds24/webhook?secret=YOUR_WEBHOOK_SECRET`;
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Beds24 Channel Manager</h1>
        <p className="text-slate-500 text-sm mt-0.5">
          Sync reservations from Booking.com and Airbnb automatically via Beds24
        </p>
      </div>
        ...
        {connected === null ? (
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-slate-700">Connected to Beds24</span>
            </div>
            <button
              onClick={disconnect}
          ...
          <div className="mt-3">
            <p className="text-sm text-slate-500 mb-4">
              In Beds24 go to <strong>Settings → Apps &amp; Integrations → API</strong>, generate an
              invite code with scopes <em>bookings (read/write)</em>, <em>properties (read)</em> and{" "}
              <em>inventory (read/write)</em>, then paste it here. Codes expire after 24 hours.
            </p>
            {error && (
      ...
      {connected && (
        <>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 mb-6">
            ...
            <p className="text-sm text-slate-500 mb-4">
              Match each StayHQ property to its Beds24 property so bookings land in the right place.
            </p>
            {loadingProps ? (
              <p className="text-sm text-slate-400">Loading properties from Beds24...</p>
            ) : (
              <>
                  ...
                  {stayhqProperties.map((p) => (
                    <div key={p.id} className="flex items-center gap-3 text-sm">
                      ...
                      >
                        ...
                        {beds24Properties.map((bp) => (
                          <option key={bp.id} value={bp.id}>{bp.name} (#{bp.id})</option>
                        ))}
                      </select>
                    </div>
                  ))}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5">
            ...
            <p className="text-sm text-slate-500 mb-3">
              For instant updates (instead of manual imports), tell Beds24 to notify StayHQ on every
              booking change. In Beds24: <strong>Settings → Properties → Access → Booking Webhooks</strong>,
              set the URL below. Replace <code className="bg-slate-100 px-1 rounded">YOUR_WEBHOOK_SECRET</code>{" "}
              with the value of the <code className="bg-slate-100 px-1 rounded">WEBHOOK_SECRET</code> variable
            ...
            <div className="flex items-center gap-2">
              <code className="flex-1 bg-slate-100 text-slate-700 text-xs px-3 py-2 rounded-lg overflow-x-auto whitespace-nowrap">
                {typeof window !== "undefined" ? window.location.origin : ""}/api/beds24/webhook?secret=YOUR_WEBHOOK_SECRET
              </code>
              <button
